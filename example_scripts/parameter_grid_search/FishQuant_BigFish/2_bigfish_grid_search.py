...
### Running parameters grid search on BigFish (FishQuant python version) - smFISH localization
### This script runs the entire parameter grid search on one image
### The script takes one argument - im_num (must be called with this argument)
### In the script, you must define the path to the parent directory of tif files
### It will run on one of the images (im_num) in the sorted list of files

### Needed conda env: https://github.com/fish-quant/big-fish
### If current BigFish version is < v0.5.0, use the develop branch
### Many thanks to Arthur Imbert & Florian Mueller!
...
### We have 83 ims - give each job an image (as a number in a sorted list below) as argument
...
## Define path to images - if RadialSymmetryLocalization repo was clones, the path should be correct  
path = '../../../documents/Simulation_of_data/'
ims_path = 'Simulated data'

    
############################################# BIG FISH FUNCTIONS ###########################################
## The below five functions are copied directly from Big-Fish!
## The minor change is that it func automated_threshold_setting retunds both the list of all thresholds and the optimal threshold
...
## Most are defined so that default BigFish settings will run in the first iteration:

## As the default sigma value depands on the dir name we set what to add to sigma:
#sigma_yx_range = np.arange(-0.5,0.51,0.25)
sigma_yx_range = [0,-0.5,-0.25,0.25,0.5]
#sigma_z_range = np.arange(-0.5,0.51,0.25)
sigma_z_range = [0,-0.5,-0.25,0.25,0.5]


## as threshold depands on the default threshold, 
# we just choose locations in the threshold array in referance to the default
# The default is the first, so it will be run for sure:
...
## Five is default, so run first
gamma_range = [5, 4, 4.5, 5.5, 6.1]

alpha_range = np.arange(0.5,0.8,0.1)

beta_range = np.arange(0.8,1.3,0.1)

# set voxel size (set to 1 as we're defining sigma):
voxel_size_z = 1
voxel_size_yx = 1
...
# Get default sigma value from dir name (simulated), or defined for embryos:

if "embryos_FISH" in im_path:
    default_sig_z = 2
    default_sig_yx = 1.5
#### As our images are simulated we give sigma instead of calculating sigma:
else:
