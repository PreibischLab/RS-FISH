...
public class RadialSymmetryBothSteps {
	...
	public static void runFullProcess1Step1Image(String root,
		String channelFilename, String experimentType, int step, int waveLength)
	{
		// TODO: exprerimentType: distinguish between N2 and SEA12:
		// so far parameters work fine on both
		// step: 1st or 2nd run of the radial symmetry
		String dataExt = ".csv";
		String imgExt = ".tif";

		// path to the csv file with RS detected centers
		File pathCenters = Paths.get(root, "centers", "all-centers" + dataExt)
			.toFile();
		// path to the images with mask
		File pathImageMask = Paths.get(root, "roi", channelFilename.substring(3) +
			imgExt).toFile();

		// path to separate channel images
		File pathImage = Paths.get(root, "channels", channelFilename + imgExt)
			.toFile();
		// path to the database with the images
		File pathDatabase = Paths.get(root, "smFISH-database", "N2-Table 1" +
			dataExt).toFile();
		// path to the median filtered images that we save
		File pathImageMedian = Paths.get(root, "median", channelFilename + imgExt)
			.toFile();
		File pathImageMedian2 = Paths.get(root, "median-2", channelFilename +
			imgExt).toFile();
		// path to save the .csv files with the results without the correction
		File pathResultCsvBeforeCorrection = Paths.get(root, "csv-before",
			channelFilename + dataExt).toFile();
		// path to save the .csv files with the results
		File pathResultCsv = Paths.get(root, "csv", channelFilename + dataExt)
			.toFile();
		// path to save the .csv files with the results
		File pathResultCsv2 = Paths.get(root, "csv-2", channelFilename + dataExt)
			.toFile();
		// path z-corrected image
		File pathZcorrected = Paths.get(root, "zCorrected", channelFilename +
			imgExt).toFile();
		// path z-corrected image
		File pathZcorrected2 = Paths.get(root, "zCorrected-2", channelFilename +
			imgExt).toFile();
		// path to files with the parameters
		File pathParameters = Paths.get(root, "csv-parameters", channelFilename +
			dataExt).toFile();

		File[] allPaths = new File[] { pathImageMask, pathImage, pathDatabase,
			pathZcorrected };
		...
		if (step == 1) {
			RunStepsPreprocess.runFirstStepPreprocessImage(pathImage, pathImageMask,
				pathImageMedian);
			RunBatchProcess.runProcessImage(pathImageMedian, pathImageMask,
				pathZcorrected, pathResultCsvBeforeCorrection, pathParameters,
				pathResultCsv, doZcorrection, waveLength);
		}
		if (step == 2) {
			RunStepsPreprocess.runSecondStepPreprocessImage(pathZcorrected,
				pathImageMask, pathCenters, pathImageMedian2);
			RunBatchProcess.runProcessImage(pathImageMedian2, pathImageMask,
				pathZcorrected2, new File(""), new File(""), pathResultCsv2,
				doZcorrection, waveLength);
		}
		if (step != 1 && step != 2) {
	...
	public static void runFullProcess1StepSEA12(int step) {
		...
		// path to save the .csv files with the results without the correction
		File pathResultCsvBeforeCorrection = Paths.get(prefix, "csv-before")
			.toFile();
		// path to save the .csv files with the results
	...
	public static void runFullProcess1StepN2(int step) {
		...
		File pathImages = Paths.get(prefix, "channels").toFile();
		// path to the database with the images
		File pathDatabase = Paths.get(prefix, "smFISH-database", "N2-Table 1.csv")
			.toFile();
		// path to the median filtered images that we save
		File pathImagesMedian = Paths.get(prefix, "median").toFile();
		File pathImagesMedian2 = Paths.get(prefix, "median-2").toFile();
