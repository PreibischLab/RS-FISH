...
public class RunBatchProcess {
	...
	{
		runProcess(pathImagesMedian, pathImagesRoi, pathDatabase, pathZcorrected,
			null, null, pathResultCsv, doZcorrection);
	}

	public static void runProcessImage(File pathMedian, File pathMask,
		File pathZcorrected, File pathResultCsvBeforeCorrection,
		File pathParameters, File pathResultCsv, boolean doZcorrection,
		int waveLength)
	{
		...
		if (pathMedian.exists()) {
			...
			GUIParams params = ParametersFirstRun.getSEA12ParametersFirstRun(
				waveLength);
			BatchProcess.process(pathMedian, pathMask, params,
				pathResultCsvBeforeCorrection, pathParameters, pathZcorrected,
		...
		File pathParameters, File pathResultCsv, boolean doZcorrection)
	{
		...
		for (ImageData imageD : imageData) {
			...
			File inputImagePath = Paths.get(pathImagesMedian.getAbsolutePath(), imageD
				.getFilename() + extImg).toFile();
			File inputRoiPath = Paths.get(pathImagesRoi.getAbsolutePath(), imageD
				.getFilename().substring(3) + extImg).toFile();

			System.out.println(NotSoUsefulOutput.toProgressString(currentIndex,
				imageData.size(), inputImagePath.getAbsolutePath()));

			if (inputImagePath.exists()) {
				// path to the image
				File outputPathZCorrected = new File("");
				if (pathZcorrected != null) outputPathZCorrected = Paths.get(
					pathZcorrected.getAbsolutePath(), imageD.getFilename() + extImg)
					.toFile();
				// table with all processing parameters
				File outputPathParameters = new File("");
				if (pathParameters != null) outputPathParameters = Paths.get(
					pathParameters.getAbsolutePath(), imageD.getFilename() + extImg)
					.toFile();
				...
				if (pathResultCsvBeforeCorrection != null)
					outputPathResultCsvBeforeCorrection = Paths.get(
						pathResultCsvBeforeCorrection.getAbsolutePath(), imageD
							.getFilename() + extOut).toFile();
				// table to store the results for each channel
				File outputPathCsv = Paths.get(pathResultCsv.getAbsolutePath(), imageD
					.getFilename() + extOut).toFile();
				// set the params according to the way length
