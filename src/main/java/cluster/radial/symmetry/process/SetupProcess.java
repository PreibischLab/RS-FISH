...
public class SetupProcess {
	...
	public static void setupProcess(String root, String datasetDbFile) {
		...
		File smFishDbFilename = Paths.get(root, "smFISH-database", datasetDbFile)
			.toFile();
		String[] folders = new String[] { "centers", "roi", "channels",
			"smFISH-database", "median", "median-2", "csv-before", "csv", "csv-2",
	...
	public static void createInputArguments(String root, String datasetDbFile,
		String fishType, boolean doFilter) throws FileNotFoundException,
		UnsupportedEncodingException
	{
		...
		ArrayList<ImageDataFull> imageDataFull = IOUtils.readDbFull(
			smFishDbFilename, doFilter);
		...
		for (ImageDataFull idf : imageDataFull) {
			if (idf.getChannels().containsKey(fishType)) {
				outputFile.println(Utils.getNameWithoutExt(idf.getChannel(fishType), idf
					.getFilename()));
			}
		}
		outputFile.close();
