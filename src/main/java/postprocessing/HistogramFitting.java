...
//				val = 0;
	...
	public static float [] runFit(Img<FloatType> img) {
		...
		// TODO: implement the background subtraction here, otherwise peakfitter will
		...
		PeakFitter<FloatType> pf = new PeakFitter<FloatType>(img, (ArrayList)peaks,
				new LevenbergMarquardtSolver(), new EllipticGaussianOrtho(), // use a
		// non-symmetric gauss (sigma_x, sigma_y, sigma_z or sigma_xy & sigma_z)
		new MLEllipticGaussianEstimator(typicalSigmas));
		pf.process();

		// there should be only one element that corresponds to the largest peak that 
