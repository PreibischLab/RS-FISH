...
public class ShowResult {
	...
	public static void ransacResultTable(final ArrayList<Spot> spots, final ArrayList<Long> timePoint, final ArrayList<Long> channelPoint, ArrayList<Float> intensity, double histThreshold) {
		...
		for (Spot spot : spots) {	
			...
			if (spot.inliers.size() != 0){ // TODO: filtered already?
				if (intensity.get(idx++) >= histThreshold){
					...
					rt.addValue("intensity",  String.format(java.util.Locale.US, "%.4f", intensity.get(idx - 1)) );
